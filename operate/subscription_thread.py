...
def _rpc_callback(request_id, request_body):
    LOGGER.debug('Received rpc message\n    Id: %s\n    Body: %s', request_id, request_body)
    try:
        ...
        if method == 'setAuto':
            ...
            if control.check_command(params):
                ...
                CLIENT.send_rpc_reply(request_id, 'failed')
                LOGGER.debug('    Malformed message or manual mode not activated')
        else:
            CLIENT.send_rpc_reply(request_id, 'failed')
            LOGGER.debug('    Malformed message')
...
def _gw_rpc_callback(self, content):
    ...
    if AUTO in method:
        if control.process_set_auto(params):
            ...
            commands[params['device']] = params['command']
            commands_lock.release()
            LOGGER.debug('Command AUTO receive success')
        else:
            LOGGER.debug('Command AUTO fail')
    elif CONTROL in method:
        if control.check_command_send_rpc(params):
            commands_lock.acquire()
            commands[params['device']] = params['command']
            commands_lock.release()
            LOGGER.debug('Command CONTROL receive success')
        else:
            LOGGER.debug('Command CONTROL malformed message or manual mode not activated')
    elif GET_STATE in method:
        if control.check_command(params):
            state = check_state_device(device, method)
            CLIENT.gw_send_rpc_reply(device, request_id, state, 1)
            LOGGER.debug('Command GET_SATE receive success: %s', state)
        else:
            LOGGER.debug('Command GET_SATE fail')
    # elif GET_VALUE in method:
    #     if control.check_command(params):
